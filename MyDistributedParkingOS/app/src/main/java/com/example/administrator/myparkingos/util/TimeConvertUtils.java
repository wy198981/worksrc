...
public class TimeConvertUtils
{
    ...
    public static String processTwo(long startMil, long endMil)
    {
        ...
        int month = c2.get(Calendar.MONTH) - c1.get(Calendar.MONTH);// 这里月可能出现负数呢?
        if (month != 0)
        {
    ...
    public static int[] processTwo(String startTime, String endTime)
    {
        long startMil = stringToLong(startTime); // 先默认转换成 formatString
        long endMil = stringToLong(endTime);

        long intervalSeconds;
        if (startMil - endMil > 0)
        {
            intervalSeconds = (startMil - endMil) / 1000;
        }
        else
        {
            intervalSeconds = (endMil - startMil) / 1000;
        }

        int[] result = new int[3];

        result[0] = (int) (intervalSeconds / (60 * 60 * 24)); // diff day
        intervalSeconds -= (60 * 60 * 24) * result[0];

        result[1] = (int) (intervalSeconds / (60 * 60)); // diff house
        intervalSeconds -= (60 * 60) * result[1];

        result[2] = (int) (intervalSeconds / (60)); // diff minute
        return result;
    }
